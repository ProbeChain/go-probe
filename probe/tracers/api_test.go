...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{
		accounts[0].addr: {Balance: big.NewInt(params.Probeer)},
		accounts[1].addr: {Balance: big.NewInt(params.Probeer)},
		accounts[2].addr: {Balance: big.NewInt(params.Probeer)},
	}}
	genBlocks := 10
	...
	var testSuite = []struct {
		...
		// Standard JSON trace upon the genesis, plain transfer.
		{
			...
			config:    nil,
			expectErr: nil,
			expect: &probeapi.ExecutionResult{
				Gas:         params.TxGas,
				Failed:      false,
				ReturnValue: "",
				StructLogs:  []probeapi.StructLogRes{},
			},
		},
...
func TestOverridenTraceCall(t *testing.T) {
	...
	var testSuite = []struct {
		...
		// Succcessful call with state overriding
		{
			...
			config: &TraceCallConfig{
				Tracer: &tracer,
				StateOverrides: &probeapi.StateOverride{
					randomAccounts[0].addr: probeapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Probeer)))},
				},
			},
			expectErr: nil,
			expect: &callTrace{
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				Tracer: &tracer,
				StateOverrides: &probeapi.StateOverride{
					randomAccounts[2].addr: probeapi.OverrideAccount{
...
func TestTraceTransaction(t *testing.T) {
	...
	if !reflect.DeepEqual(result, &probeapi.ExecutionResult{
		Gas:         params.TxGas,
		Failed:      false,
		ReturnValue: "",
		StructLogs:  []probeapi.StructLogRes{},
	...
	var testSuite = []struct {
		...
		// Trace head block
		{
			...
			expect: []*txTraceResult{
				{
					Result: &probeapi.ExecutionResult{
						Gas:         params.TxGas,
						Failed:      false,
						ReturnValue: "",
						StructLogs:  []probeapi.StructLogRes{},
