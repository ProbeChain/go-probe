...
func (api *PublicProbeumAPI) Hashrate() hexutil.Uint64 {
...
func NewPublicMinerAPI(e *Probeum) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Probeum) *PrivateMinerAPI {
...
// PrivateAdminAPI is the collection of Probeum full node-related APIs
...
func (api *PrivateAdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.probe.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.probe.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.probe.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.probe.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.probe.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Probeum full node APIs exposed
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.probe.miner.Pending()
		return stateDb.RawDump(opts), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.probe.blockchain.CurrentBlock()
	} else {
		block = api.probe.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.probe.BlockChain().StateAt(block.Root())
	if err != nil {
...
// PrivateDebugAPI is the collection of Probeum full node APIs exposed over
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.probe.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		err     error
		blocks  = rawdb.ReadAllBadBlocks(api.probe.chainDb)
	...
	for _, block := range blocks {
		...
		if blockJSON, err = probeapi.RPCMarshalBlock(block, true, true); err != nil {
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			if number == rpc.LatestBlockNumber {
				...
				block = api.probe.blockchain.GetBlockByNumber(uint64(number))
			}
			if block == nil {
		...
		block := api.probe.blockchain.GetBlockByHash(hash)
		if block == nil {
...
func (api *PrivateDebugAPI) StorageRangeAt(blockHash common.Hash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	// Retrieve the block
	block := api.probe.blockchain.GetBlockByHash(blockHash)
	if block == nil {
		return StorageRangeResult{}, fmt.Errorf("block %#x not found", blockHash)
	}
	_, _, statedb, err := api.probe.stateAtTransaction(block, txIndex, 0)
	if err != nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.probe.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.probe.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.probe.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.probe.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.probe.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.probe.BlockChain().StateCache().TrieDB()
