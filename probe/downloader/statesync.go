...
// stateReq represents a batch of state fetch requests grouped togprobeer into
...
type stateReq struct {
	...
	dropped   bool                      // Flag whprobeer the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somprobeing else, ignore the stale timeout.
...
func (s *stateSync) loop() (err error) {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somprobeing to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < probedb.IdealBatchSize {
		return nil
	}
	start := time.Now()
	...
	for _, p := range peers {
		...
		if len(nodes)+len(codes) > 0 {
			...
			case s.d.trackStateReq <- req:
				req.peer.FetchNodeData(append(nodes, codes...)) // Unified retrieval under probe/6x
			case <-s.cancel:
			case <-s.d.cancelCh:
			}
		}
...
// delivered. Returns whprobeer the peer actually managed to deliver anything of
...
func (s *stateSync) process(req *stateReq) (int, error) {
	...
	for hash, task := range req.trieTasks {
		// If the node did deliver somprobeing, missing items may be due to a protocol
...
// peer into the state trie, returning whprobeer anything useful was written or any
// error occurred.
