...
type fetchRequest struct {
	...
	From    uint64          // [probe/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [probe/62] Requested headers, sorted by request order
...
// InFlightHeaders retrieves whprobeer there are header fetch requests currently
// in flight.
...
// InFlightBlocks retrieves whprobeer there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whprobeer there are receipt fetch requests currently
// in flight.
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading somprobeing (sanity check to
	// not corrupt state)
...
// returns a flag whprobeer empty blocks were queued requiring processing.
...
// also returns a flag whprobeer empty receipts were queued requiring importing.
...
//   progress - whprobeer any progress was made
//   throttle - if the caller should throttle for a while
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, kind uint) (*fetchRequest, bool, bool) {
	// Short circuit if the pool has been depleted, or if the peer's already
	// downloading somprobeing (sanity check not to corrupt state)
	if taskQueue.Empty() {
...
func (q *queue) deliver(id string, taskPool map[common.Hash]*types.Header,
	...
	for _, header := range request.Headers[:i] {
		if res, stale, err := q.resultCache.GetDeliverySlot(header.Number.Uint64()); err == nil {
			...
			// not somprobeing to panic about
