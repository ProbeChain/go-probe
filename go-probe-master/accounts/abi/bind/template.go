...
type tmplContract struct {
	...
	Library     bool                   // Indicator whprobeer the contract is a library
}
...
type tmplMethod struct {
	...
	Structured bool       // Whprobeer the returns should be accumulated into a struct
}
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new Probeum contract, binding an instance of {{.Type}} to it.
	...
	{{range .Events}}
		...
		type {{$contract.Type}}{{.Normalized.Name}}Iterator struct {
			...
			sub  probeum.Subscription // Subscription for errors, completion and termination
			done bool                  // Whprobeer the subscription completed delivering logs
			fail error                 // Occurred error to stop iteration
		}
		// Next advances the iterator to the subsequent event, returning whprobeer there
...
{{if not .Library}}public {{end}}class {{.Type}} {
	...
	// deploy deploys a new Probeum contract, binding an instance of {{.Type}} to it.
	public static {{.Type}} deploy(TransactOpts auth, ProbeumClient client{{range .Constructor.Inputs}}, {{bindtype .Type $structs}} {{.Name}}{{end}}) throws Exception {
		Interfaces args = Gprobe.newInterfaces({{(len .Constructor.Inputs)}});
		...
		{{end}}
		{{end}}
		{{range $index, $element := .Constructor.Inputs}}Interface arg{{$index}} = Gprobe.newInterface();arg{{$index}}.set{{namedtype (bindtype .Type $structs) .Type}}({{.Name}});args.set({{$index}},arg{{$index}});
		{{end}}
		return new {{.Type}}(Gprobe.deployContract(auth, ABI, Gprobe.decodeFromHex(bytecode), client, args));
	}
	...
	public {{.Type}}(Address address, ProbeumClient client) throws Exception {
		this(Gprobe.bindContract(address, ABI, client));
	}
	...
	public {{if gt (len .Normalized.Outputs) 1}}{{capitalise .Normalized.Name}}Results{{else if eq (len .Normalized.Outputs) 0}}void{{else}}{{range .Normalized.Outputs}}{{bindtype .Type $structs}}{{end}}{{end}} {{.Normalized.Name}}(CallOpts opts{{range .Normalized.Inputs}}, {{bindtype .Type $structs}} {{.Name}}{{end}}) throws Exception {
		Interfaces args = Gprobe.newInterfaces({{(len .Normalized.Inputs)}});
		{{range $index, $item := .Normalized.Inputs}}Interface arg{{$index}} = Gprobe.newInterface();arg{{$index}}.set{{namedtype (bindtype .Type $structs) .Type}}({{.Name}});args.set({{$index}},arg{{$index}});
		{{end}}

		Interfaces results = Gprobe.newInterfaces({{(len .Normalized.Outputs)}});
		{{range $index, $item := .Normalized.Outputs}}Interface result{{$index}} = Gprobe.newInterface(); result{{$index}}.setDefault{{namedtype (bindtype .Type $structs) .Type}}(); results.set({{$index}}, result{{$index}});
		{{end}}

		if (opts == null) {
			opts = Gprobe.newCallOpts();
		}
