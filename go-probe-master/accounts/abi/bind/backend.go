...
type ContractTransactor interface {
	...
	EstimateGas(ctx context.Context, call probeum.CallMsg) (gas uint64, err error)
...
type ContractFilterer interface {
	...
	FilterLogs(ctx context.Context, query probeum.FilterQuery) ([]types.Log, error)

	// SubscribeFilterLogs creates a background log filtering operation, returning
	// a subscription immediately, which can be used to stream the found events.
	SubscribeFilterLogs(ctx context.Context, query probeum.FilterQuery, ch chan<- types.Log) (probeum.Subscription, error)
}
