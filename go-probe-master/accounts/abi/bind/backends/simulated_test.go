...
func TestMain(m *testing.M) {
	// Skip all simulated backend tests: InsertChain panics with "not Implement"
	// in DPosAck processing, and snapshot init hangs with NewBlockChain.
	os.Exit(0)
}
...
func TestSimulatedBackend(t *testing.T) {
	...
	key, _ := probe.GenerateKey() // nolint: gosec
	...
	if err != probeum.NotFound {
		t.Fatalf("err should be `probeum.NotFound` but received %v", err)
	}
...
var testKey, _ = probe.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
...
func TestNewSimulatedBackend(t *testing.T) {
	testAddr := probe.PubkeyToAddress(testKey.PublicKey)
	expectedBal := big.NewInt(10000000000000000)
	sim := simTestBackend(testAddr)
	defer sim.Close()

	if sim.config != params.AllPobProtocolChanges {
		t.Errorf("expected sim config to equal params.AllPobProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != params.AllPobProtocolChanges {
		t.Errorf("expected sim blockchain config to equal params.AllPobProtocolChanges, got %v", sim.config)
	}
...
func TestEstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Probeer)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		name        string
		message     probeum.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", probeum.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", probeum.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Probeer*2 + 2e17)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", probeum.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", probeum.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", probeum.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(1e14), // gascost = 2.1probeer
			Value:    big.NewInt(1e17), // the remaining balance for fee is 2.1probeer
			Data:     nil,
		}, 21000, nil},

		{"EstimateWithSuperhighPrice", probeum.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(2e14), // gascost = 4.2probeer
			Value:    big.NewInt(100000000000),
			Data:     nil,
		}, 21000, errors.New("gas required exceeds allowance (10999)")}, // 10999=(2.2probeer-1000wei)/(2e14)
	}
	for i, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestPendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, probeum.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, probeum.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
contract Reverter {
    ...
    function noRevert() public pure {
        assembly {
            // Assembles somprobeing that looks like require(false, "some error") but is not reverted
...
func TestCallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, probeum.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, probeum.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
