...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-probe/master/doc/probeapp.asc
...
type ledgerDriver struct {
	...
	browser bool          // Flag whprobeer the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Probeum app in browser mode", w.failure
	}
	if w.offline() {
		return "Probeum app offline", w.failure
	}
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Probeum app is not running or in browser mode, nothing more to do, return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whprobeer legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
