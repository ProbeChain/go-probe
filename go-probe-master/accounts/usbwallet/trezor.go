...
import (
	...
	"github.com/probechain/go-probe/accounts/usbwallet/trezor"
...
type trezorDriver struct {
	...
	pinwait        bool          // Flags whprobeer the device is waiting for PIN entry
	passphrasewait bool          // Flags whprobeer the device is waiting for passphrase entry
...
// Status implements accounts.Wallet, always whprobeer the Trezor is opened, closed
// or whprobeer the ProbeChain app was not started on it.
...
// ProbeChain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.ProbeumAddress)
	if _, err := w.trezorExchange(&trezor.ProbeumGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.ProbeumTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.ProbeumTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
