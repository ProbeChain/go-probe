...
type Wallet struct {
	...
	deriveChain     probeum.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// but we can't return ErrPINNeeded directly here because we don't know whprobeer
			// a PIN check or a PIN reset is needed.
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whprobeer
...
// Contains returns whprobeer an account is part of this particular wallet or not.
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain probeum.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
type Session struct {
	...
	verified bool                  // Whprobeer the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whprobeer the card has been initialized with a private key
}
