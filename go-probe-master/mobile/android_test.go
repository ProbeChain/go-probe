...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gprobe.LightScryptN, Gprobe.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.probeum", new NodeConfig());
			node.start();
			...
			ProbeumClient ec = node.getProbeumClient();
	...
	// signatures too. Regression test for go-probeum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogprobeer
	...
	workspace, err := ioutil.TempDir("", "gprobe-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/probeum/gprobetest", "libs"} {
	...
	cp.CopyFile(filepath.Join("libs", "gprobe.aar"), "gprobe.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "probeum", "gprobetest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.probeum.gprobetest"
