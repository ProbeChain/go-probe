...
// Package gprobe contains the simplified mobile APIs to go-probeum.
//
// The scope of this package is *not* to allow writing a custom Probeum client
// with pieces plucked from go-probeum, rather to allow writing native dapps on
...
// from the go-probeum repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-probeum version.
