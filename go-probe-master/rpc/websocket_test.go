...
// This test checks whprobeer calls exceeding the request size limit are rejected.
...
func TestClientWebsocketPing(t *testing.T) {
	...
	sub, err := client.ProbeSubscribe(ctx, resultChan, "foo")
	if err != nil {
	...
	// This is important for reproducing https://github.com/probechain/go-probe/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the probe_subscribe call in TestClientWebsocketPing.
	const (
