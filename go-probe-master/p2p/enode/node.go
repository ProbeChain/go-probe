...
func compressStr(str string) string {
	//validatorEnodeTemp := common.BytesToValidatorEnode([]byte(str))
	validatorEnodeTemp := []byte(str)
	validatorEnode := bytes.Trim(validatorEnodeTemp[:], "\x00")
	return string(validatorEnode[:])
}
...
// ValidateComplete checks whprobeer n has a valid IP and UDP port.
