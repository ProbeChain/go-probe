...
import (
	...
	probeproto "github.com/probechain/go-probe/probe/protocols/probe"
...
// backend encompasses the bare-minimum functionality needed for probestats reporting
...
// Service implements a ProbeChain netstats reporting daemon that pushes local
...
// parseProbestatsURL parses the netstats connection url.
...
func parseProbestatsURL(url string) (parts []string, err error) {
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	parts, err := parseProbestatsURL(url)
	if err != nil {
		return err
	}
	probestats := &Service{
	...
	node.RegisterLifecycle(probestats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["probe"]; info != nil {
		network = fmt.Sprintf("%d", info.(*probeproto.NodeInfo).Network)
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the probestats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to probestats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to probestats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to probestats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to probestats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to probestats")
