...
// importGenesis imports a Gprobe genesis spec into puppprobe.
...
func (w *wizard) manageGenesis() {
	// Figure out whprobeer to modify or export the genesis
	fmt.Println()
	...
	case "1":
		...
		fmt.Println()
		fmt.Printf("Which block should Shenzhen come into effect? (default = %v)\n", w.conf.Genesis.Config.ShenzhenBlock)
		w.conf.Genesis.Config.ShenzhenBlock = w.readDefaultBigInt(w.conf.Genesis.Config.ShenzhenBlock)
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alprobe.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppprobe and Gprobe
		gprobeJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile(gprobeJson, out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gprobeJson)

		// Export the genesis spec used by Alprobe (formerly C++ Probeum)
		if spec, err := newAlprobeGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Alprobe chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alprobe", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly ProbeumJ)
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somprobeing I can do")
		return
	}
}
