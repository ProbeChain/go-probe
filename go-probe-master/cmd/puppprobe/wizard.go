...
// config contains all the configurations needed by puppprobe that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	probestats  string   // Probestats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppprobe configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Probeum services known to be running on servers
...
// converts it to an ProbeChain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by probestats, which compares IPs textually, not by value.
