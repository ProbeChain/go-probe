...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"probestats", "explorer", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "probestats":
				if infos, err := checkProbestats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have probestats running, ask whprobeer to make the secret public or not
	if w.conf.probestats != "" {
		fmt.Println()
		fmt.Println("Include probestats secret on dashboard (y/n)? (default = yes)")
