...
FROM probeum/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--probestats", "{{.Probestats}}", "--probeport", "{{.ProbePort}}",     \
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Funding amount (base tier)":   fmt.Sprintf("%d Probeers", info.amount),
...
// checkFaucet does a health-check against a faucet server to verify whprobeer
