...
FROM puppprobe/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'gprobe --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gprobe --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.ProbePort}} --bootnodes {{.Bootnodes}} --probestats \'{{.Probestats}}\' --cache=512 --http --http.api "net,web3,probe,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gprobe --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.ProbePort}} --bootnodes {{.Bootnodes}} --probestats \'{{.Probestats}}\' --cache=512 --http --http.api "net,web3,probe,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.ProbePort}}:{{.ProbePort}}"
            - "{{.ProbePort}}:{{.ProbePort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - ETH_PORT={{.ProbePort}}
            - ETH_NAME={{.ProbeName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Probestats":  config.node.probestats,
		"ProbePort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"ProbePort":     config.node.port,
		"ProbeName":     config.node.probestats[:strings.Index(config.node.probestats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Probeum listener port ": strconv.Itoa(info.node.port),
		"Probestats username":       info.node.probestats,
	}
	return report
}

// checkExplorer does a health-check against a block explorer server to verify
// whprobeer it's running, and if yes, whprobeer it's responsive.
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.probeum"],
			port:     infos.portmap[infos.envvars["ETH_PORT"]+"/tcp"],
			probestats: infos.envvars["ETH_NAME"],
		},
