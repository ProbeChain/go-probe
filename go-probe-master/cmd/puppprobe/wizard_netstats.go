...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for probestats availability")
	if infos, err := checkProbestats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["probestats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["probestats"] = infos.Report()
		probestats = infos.config
	}
