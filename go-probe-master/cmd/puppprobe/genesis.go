...
import (
	...
	math2 "github.com/probeum/go-probeum/common/math"
...
// alprobeGenesisSpec represents the genesis specification format used by the
// C++ Probeum implementation.
type alprobeGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alprobeGenesisSpecAccount `json:"accounts"`
}

// alprobeGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alprobeGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance,omitempty"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alprobeGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alprobeGenesisSpecBuiltin is the precompiled contract definition.
type alprobeGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock *hexutil.Big                   `json:"startingBlock,omitempty"`
	Linear        *alprobeGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alprobeGenesisSpecLinearPricing struct {
...
func newAlprobeGenesisSpec(network string, genesis *core.Genesis) (*alprobeGenesisSpec, error) {
	...
	// config but alprobe will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.setPrecompile(1, &alprobeGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alprobeGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alprobeGenesisSpecBuiltin{Name: "sha256",
		Linear: &alprobeGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alprobeGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alprobeGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alprobeGenesisSpecBuiltin{Name: "identity",
		Linear: &alprobeGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alprobeGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &alprobeGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alprobeGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alprobeGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alprobeGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alprobeGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &alprobeGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Alprobe hardcoded the gas policy
		spec.setPrecompile(7, &alprobeGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Alprobe hardcoded the gas policy
		spec.setPrecompile(9, &alprobeGenesisSpecBuiltin{
...
func (spec *alprobeGenesisSpec) setPrecompile(address byte, data *alprobeGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alprobeGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alprobeGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alprobeGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alprobeGenesisSpecAccount)
	}
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-probeum/pull/11039,
...
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	...
	spec.Engine.Probeash.Params.BlockReward = make(map[string]string)
	spec.Engine.Probeash.Params.DifficultyBombDelays = make(map[string]string)
	// Frontier
	...
	// https://github.com/probeum/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/probeum/EIPs/blob/master/EIPS/eip-607.md
	...
	// gprobe has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Probeum.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.Probeum.MixHash = genesis.Mixhash[:]
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyProbeumGenesisSpec represents the genesis specification format used by the
// Python Probeum implementation.
type pyProbeumGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
