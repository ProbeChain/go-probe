...
func (w *wizard) deployNode(boot bool) {
	...
	if infos.probestats == "" {
		...
		infos.probestats = w.readString() + ":" + w.conf.probestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.probestats)
		infos.probestats = w.readDefaultString(infos.probestats) + ":" + w.conf.probestats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Probeum node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
