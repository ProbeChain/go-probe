...
// deployNode deploys a new Probeum node container to a remote machine via SSH,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Probestats username":        info.probestats,
	}
	if info.gasTarget > 0 {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gprobe --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
