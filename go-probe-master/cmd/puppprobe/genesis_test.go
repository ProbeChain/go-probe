...
func TestAlprobeSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gprobe.json")
	if err != nil {
	...
	spec, err := newAlprobeGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alprobe.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alprobeGenesisSpec{}
...
// Tests the go-probeum to Parity chainspec conversion for the Stureby testnet.
