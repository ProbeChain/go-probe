...
func (w *wizard) deployExplorer() {
	...
	if w.conf.probestats == "" {
		log.Error("No probestats server configured")
		return
	}
	...
	fmt.Println()
	if infos.node.probestats == "" {
		fmt.Printf("What should the explorer be called on the stats page?\n")
		infos.node.probestats = w.readString() + ":" + w.conf.probestats
	} else {
		fmt.Printf("What should the explorer be called on the stats page? (default = %s)\n", infos.node.probestats)
		infos.node.probestats = w.readDefaultString(infos.node.probestats) + ":" + w.conf.probestats
	}
