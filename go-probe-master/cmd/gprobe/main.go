...
// gprobe is the official command-line client for Probeum.
package main
...
const (
	clientIdentifier = "gprobe" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-probeum command line interface")
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.RopstenFlag.Name):
		log.Info("Starting Gprobe on Ropsten testnet...")

	case ctx.GlobalIsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Gprobe on Rinkeby testnet...")

	case ctx.GlobalIsSet(utils.GoerliFlag.Name):
		log.Info("Starting Gprobe on Görli testnet...")

	case ctx.GlobalIsSet(utils.CalaverasFlag.Name):
		log.Info("Starting Gprobe on Calaveras testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Gprobe in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Gprobe on Probeum mainnet...")
	}
...
// gprobe is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gprobe(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend probeapi.Backend) {
	...
	// Create a client to interact with local gprobe node.
	rpcClient, err := stack.Attach()
	if err != nil {
		utils.Fatalf("Failed to attach to self: %v", err)
	}
	probeClient := probeclient.NewClient(rpcClient)
	...
	go func() {
		...
		for event := range events {
			...
			case accounts.WalletOpened:
				...
				event.Wallet.SelfDerive(derivationPaths, probeClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Probeum node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		probeBackend, ok := backend.(*probe.ProbeAPIBackend)
		if !ok {
			utils.Fatalf("Probeum service not running: %v", err)
		}
		// Set the gas price to the limits from the CLI and start mining
		gasprice := utils.GlobalBig(ctx, utils.MinerGasPriceFlag.Name)
		probeBackend.TxPool().SetGasPrice(gasprice)
		// start mining
		threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name)
		if err := probeBackend.StartMining(threads); err != nil {
