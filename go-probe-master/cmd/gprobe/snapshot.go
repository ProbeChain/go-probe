...
var (
	snapshotCommand = cli.Command{
		...
		Subcommands: []cli.Command{
			{
				Name:      "prune-state",
				Usage:     "Prune stale probeum state data based on the snapshot",
				ArgsUsage: "<root>",
				...
				Description: `
gprobe snapshot prune-state <state-root>
...
during the use of Gprobe, please also specify it here for correct deletion. Otherwise
...
`,
			},
			{
				...
				Description: `
gprobe snapshot verify-state <state-root>
...
`,
			},
			{
				...
				Description: `
gprobe snapshot traverse-state <state-root>
...
`,
			},
			{
				...
				Description: `
gprobe snapshot traverse-rawstate <state-root>
...
`,
			...
			{
				Name:      "dump",
				Usage:     "Dump a specific block from storage (same as 'gprobe dump' but using snapshots)",
				...
				Description: `
This command is semantically equivalent to 'gprobe dump', but uses the snapshots
...
func pruneState(ctx *cli.Context) error {
	...
	pruner, err := pruner.NewPruner(chaindb, stack.ResolvePath(""), stack.ResolvePath(config.Probe.TrieCleanCacheJournal), ctx.GlobalUint64(utils.BloomFilterSizeFlag.Name))
	if err != nil {
