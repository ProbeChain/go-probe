...
// Package gprobeclient provides an RPC client for gprobe-specific APIs.
...
// Client is a wrapper around rpc.Client that implements gprobe-specific functionality.
//
// If you want to use the standardized Probeum RPC functionality, use probeclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg probeum.CallMsg) (*types.AccessList, uint64, string, error) {
	...
	if err := ec.c.CallContext(ctx, &result, "probe_createAccessList", toCallArg(msg)); err != nil {
		return nil, 0, "", err
	}
...
func (ec *Client) GetProof(ctx context.Context, account common.Address, keys []string, blockNumber *big.Int) (*AccountResult, error) {
	...
	err := ec.c.CallContext(ctx, &res, "probe_getProof", account, keys, toBlockNumArg(blockNumber))
...
// the message call.
// Please use probeclient.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContract(ctx context.Context, msg probeum.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
	var hex hexutil.Bytes
	err := ec.c.CallContext(
		ctx, &hex, "probe_call", toCallArg(msg),
		toBlockNumArg(blockNumber), toOverrideMap(overrides),
	)
	return hex, err
}

// GCStats retrieves the current garbage collection stats from a gprobe node.
...
// MemStats retrieves the current memory stats from a gprobe node.
...
// GetNodeInfo retrieves the node info of a gprobe node.
...
func (ec *Client) SubscribePendingTransactions(ctx context.Context, ch chan<- common.Hash) (*rpc.ClientSubscription, error) {
	return ec.c.ProbeSubscribe(ctx, ch, "newPendingTransactions")
}
