...
// and negative banalce. Boprobe positive balance and negative balance will decrease
...
func NewClientPool(balanceDb probedb.KeyValueStore, minCap uint64, connectedBias time.Duration, clock mclock.Clock, synced func() bool) *ClientPool {
...
func (cp *ClientPool) SetCapacity(node *enode.Node, reqCap uint64, bias time.Duration, requested bool) (capacity uint64, err error) {
	...
	cp.ns.Operation(func() {
		...
		if requested {
			...
			// whprobeer the update is the direct result of a SetCapacity call on the given node
			cp.capReqNode = node
			defer func() {
				cp.capReqNode = nil
			}()
