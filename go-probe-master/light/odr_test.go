...
type testOdr struct {
	...
	sdb, ldb      probedb.Database
	disable       bool
}

func (odr *testOdr) Database() probedb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db probedb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes2(t *testing.T) {
	t.Skip("skipped: block generation fails with PoB changes")
	testChainOdr(t, 1, odrGetBlock)
}

func odrGetBlock(ctx context.Context, db probedb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db probedb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db probedb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db probedb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
	...
	for i := 0; i < 3; i++ {
		...
		msg := callmsg{types.NewMessage(testBankAddress, &testContractAddr, 1000, big.NewInt(params.InitialBaseFee), 1000000, big.NewInt(params.InitialBaseFee), new(big.Int), new(big.Int), data, nil, false)}
...
func testChainGen(i int, block *core.BlockGen) {
	...
	switch i {
	case 0:
		// In block 1, the test bank sends account #1 some probeer.
		...
		block.AddTx(tx)
	case 1:
		// In block 2, the test bank sends some more probeer to account #1.
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	...
	blockchain, _ := core.NewBlockChain(sdb, nil, params.TestChainConfig, pob.NewFullFaker(), vm.Config{}, nil, nil, nil)
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, pob.NewFaker(), sdb, 4, testChainGen)
	if _, err := blockchain.InsertChain(gchain); err != nil {
		t.Fatal(err)
	}

	odr := &testOdr{sdb: sdb, ldb: ldb, indexerConfig: TestClientIndexerConfig}
	lightchain, err := NewLightChain(odr, params.TestChainConfig, pob.NewFullFaker(), nil)
	if err != nil {
		t.Fatal(err)
	}
