...
type OdrRequest interface {
	StoreResult(db probedb.Database)
}
...
func (req *TrieRequest) StoreResult(db probedb.Database) {
	req.Proof.Store(db)
}
...
// StoreResult stores the retrieved data in local database
func (req *CodeRequest) StoreResult(db probedb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *BlockRequest) StoreResult(db probedb.Database) {
	rawdb.WriteBodyRLP(db, req.Hash, req.Number, req.Rlp)
}

// ReceiptsRequest is the ODR request type for retrieving receipts.
type ReceiptsRequest struct {
	Untrusted bool // Indicator whprobeer the result retrieved is trusted or not
	Hash      common.Hash
	Number    uint64
	Header    *types.Header
...
// StoreResult stores the retrieved data in local database
func (req *ReceiptsRequest) StoreResult(db probedb.Database) {
	if !req.Untrusted {
...
// StoreResult stores the retrieved data in local database
func (req *ChtRequest) StoreResult(db probedb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *BloomRequest) StoreResult(db probedb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *TxStatusRequest) StoreResult(db probedb.Database) {}
