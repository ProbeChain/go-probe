...
import (
	...
	l "github.com/probechain/go-probe/les"
...
var (
	...
	bankFunds  = new(big.Int).Mul(big.NewInt(100), big.NewInt(params.Probeer))
...
func makechain() (bc *core.BlockChain, addrHashes, txHashes []common.Hash) {
	...
	blocks, _ := core.GenerateChain(gspec.Config, genesis, probeash.NewFaker(), db, testChainLen,
		func(i int, gen *core.BlockGen) {
			...
			if i%4 == 0 {
				...
				tx, _ = types.SignTx(types.NewTransaction(nonce, addr, big.NewInt(10000), params.TxGas, big.NewInt(params.GPico), nil), signer, bankKey)
			}
			gen.AddTx(tx)
			addrHashes = append(addrHashes, crypto.Keccak256Hash(addr[:]))
			txHashes = append(txHashes, tx.Hash())
		})
	bc, _ = core.NewBlockChain(db, nil, gspec.Config, probeash.NewFaker(), vm.Config{}, nil, nil)
