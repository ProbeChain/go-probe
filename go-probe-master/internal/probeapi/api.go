...
// PublicProbeumAPI provides an API to access Probeum related information.
...
func (s *PublicProbeumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicProbeumAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicProbeumAPI) FeeHistory(ctx context.Context, blockCount int, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (feeHistoryResults, error) {
...
func (s *PublicProbeumAPI) Syncing() (interface{}, error) {
...
//
// https://github.com/probechain/go-probe/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with probe_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Probeum Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/probechain/go-probe/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Probeum signature (V is not 27 or 28)")
	}
...
// ChainId is the EIP-155 replay-protection chain id for the current probeum chain config.
...
//
// https://github.com/probeum/wiki/wiki/JSON-RPC#probe_sign
...
// Version returns the current probeum protocol version.
func (s *PublicNetAPI) Version() string {
	return fmt.Sprintf("%d", s.networkVersion)
}

// checkTxFee is an internal function used to check whprobeer the fee of
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeProbe := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Probeer)))
	feeFloat, _ := feeProbe.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f probeer) exceeds the configured cap (%.2f probeer)", feeFloat, cap)
	}
	return nil
}
