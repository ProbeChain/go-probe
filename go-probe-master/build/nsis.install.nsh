Name "gprobe ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install gprobe binary
Section "Gprobe" GETH_IDX
  SetOutPath $INSTDIR
  file {{.Gprobe}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\gprobe.exe"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\gprobe.exe" "attach"
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe"

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "Gprobe incoming peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Gprobe outgoing peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Gprobe UDP discovery (UDP:30303)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "Gprobe incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\gprobe.exe" "" "" "Probeum" 30303 "" "" ""
  SimpleFC::AdvAddRule "Gprobe outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\gprobe.exe" "" "" "Probeum" "" 30303 "" ""
  SimpleFC::AdvAddRule "Gprobe UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\gprobe.exe" "" "" "Probeum" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/probeum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\gprobe.ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\gprobe.ipc"
