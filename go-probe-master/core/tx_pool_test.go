...
func transaction(nonce uint64, gaslimit uint64, key *probe.PrivateKey) *types.Transaction {
	return pricedTransaction(nonce, gaslimit, big.NewInt(1), key)
}

func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *probe.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.HomesteadSigner{}, key)
	return tx
}

func pricedDataTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *probe.PrivateKey, bytes uint64) *types.Transaction {
...
func dynamicFeeTx(nonce uint64, gaslimit uint64, gasFee *big.Int, tip *big.Int, key *probe.PrivateKey) *types.Transaction {
...
func setupTxPool() (*TxPool, *probe.PrivateKey) {
	return setupTxPoolWithConfig(params.TestChainConfig)
}

func setupTxPoolWithConfig(config *params.ChainConfig) (*TxPool, *probe.PrivateKey) {
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Probeer))
		*c.trigger = false
	}
	return stdb, nil
}
...
// state reset and tests whprobeer the pending state is in sync with the
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Probeer))
...
func TestTransactionDropping(t *testing.T) {
	...
	account := probe.PubkeyToAddress(key.PublicKey)
...
func TestTransactionPostponing(t *testing.T) {
	...
	keys := make([]*probe.PrivateKey, 2)
	accs := make([]common.Address, len(keys))

	for i := 0; i < len(keys); i++ {
		keys[i], _ = probe.GenerateKey()
		accs[i] = probe.PubkeyToAddress(keys[i].PublicKey)

		testAddBalance(pool, probe.PubkeyToAddress(keys[i].PublicKey), big.NewInt(50100))
	}
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	keys := make([]*probe.PrivateKey, 5)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = probe.GenerateKey()
		testAddBalance(pool, probe.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000000))
	}
	...
	if nolocals {
		...
		if queued := pool.queue[probe.PubkeyToAddress(local.PublicKey)].Len(); uint64(queued) != 3*config.GlobalQueue {
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	local, _ := probe.GenerateKey()
	remote, _ := probe.GenerateKey()

	testAddBalance(pool, probe.PubkeyToAddress(local.PublicKey), big.NewInt(1000000000))
	testAddBalance(pool, probe.PubkeyToAddress(remote.PublicKey), big.NewInt(1000000000))
	...
	statedb.SetNonce(probe.PubkeyToAddress(remote.PublicKey), 2)
	statedb.SetNonce(probe.PubkeyToAddress(local.PublicKey), 2)
	<-pool.requestReset(nil, nil)
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	// Create a number of test accounts and fund them
	keys := make([]*probe.PrivateKey, 3)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = probe.GenerateKey()
		testAddBalance(pool, probe.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000*1000000))
	}
...
// Tests whprobeer highest fee cap transaction is retained after a batch of high effective
...
func TestDualHeapEviction(t *testing.T) {
	...
	add := func(urgent bool) {
		...
		for i := range txs {
			...
			key, _ := probe.GenerateKey()
			testAddBalance(pool, probe.PubkeyToAddress(key.PublicKey), big.NewInt(1000000000000))
			if urgent {
...
func TestTransactionDeduplication(t *testing.T) {
	...
	key, _ := probe.GenerateKey()
	testAddBalance(pool, probe.PubkeyToAddress(key.PublicKey), big.NewInt(1000000000))
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	pool.Stop()
	statedb.SetNonce(probe.PubkeyToAddress(local.PublicKey), 1)
	blockchain = &testBlockChain{statedb, 1000000, new(event.Feed)}
...
func BenchmarkInsertRemoteWithAllLocals(b *testing.B) {
	...
	remoteKey, _ := probe.GenerateKey()
	remoteAddr := probe.PubkeyToAddress(remoteKey.PublicKey)
