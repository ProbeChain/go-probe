...
type freezerdb struct {
	probedb.KeyValueStore
	probedb.AncientStore
}
...
func NewDatabase(db probedb.KeyValueStore) probedb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db probedb.KeyValueStore, freezer string, namespace string, readonly bool) (probedb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains somprobeing, ensure that the genesis blocks
...
func NewMemoryDatabase() probedb.Database {
...
func NewMemoryDatabaseWithCap(size int) probedb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string, readonly bool) (probedb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string, readonly bool) (probedb.Database, error) {
...
func InspectDatabase(db probedb.Database, keyPrefix, keyStart []byte) error {
