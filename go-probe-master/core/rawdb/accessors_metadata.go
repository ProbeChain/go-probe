...
func ReadDatabaseVersion(db probedb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db probedb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db probedb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db probedb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db probedb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db probedb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
