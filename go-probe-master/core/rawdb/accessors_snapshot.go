...
func ReadSnapshotDisabled(db probedb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db probedb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db probedb.KeyValueWriter) {
...
func ReadSnapshotRoot(db probedb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db probedb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db probedb.KeyValueWriter) {
...
func ReadAccountSnapshot(db probedb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db probedb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db probedb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db probedb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db probedb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db probedb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db probedb.Iteratee, accountHash common.Hash) probedb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db probedb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db probedb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db probedb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db probedb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db probedb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db probedb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db probedb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db probedb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db probedb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db probedb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db probedb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSnapshotSyncStatus(db probedb.KeyValueWriter) {
