...
func (v *BlockValidator) ValidateBody(block *types.Block) error {
	// Check whprobeer the block's known, and if not, that it's linkable
	...
	if err := v.engine.VerifyUncleBehaviorProofs(v.bc, block); err != nil {
		log.Error("ValidateBody", "VerifyUncleBehaviorProofs error", err)
		return err
	}
	if err := v.engine.VerifyValidatorInfo(v.bc, block); err != nil {
		log.Error("ValidateBody", "VerifyValidatorInfo error", err)
		return err
	}
