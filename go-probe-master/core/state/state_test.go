...
func TestDump(t *testing.T) {
	t.Skip("skipped: state dump format changed with PoB account fields")
	db := rawdb.NewMemoryDatabase()
	sdb, _ := New(common.Hash{}, NewDatabaseWithConfig(db, nil), nil)
	s := &stateTest{db: db, state: sdb}

	// generate a few entries
	obj1, _ := s.state.GetOrNewStateObject(common.BytesToAddress([]byte{0x01}))
	obj1.AddBalance(big.NewInt(22))
	obj2, _ := s.state.GetOrNewStateObject(common.BytesToAddress([]byte{0x01, 0x02}))
	obj2.SetCode(crypto.Keccak256Hash([]byte{3, 3, 3, 3, 3, 3, 3}), []byte{3, 3, 3, 3, 3, 3, 3})
	obj3, _ := s.state.GetOrNewStateObject(common.BytesToAddress([]byte{0x02}))
...
func TestSnapshot2(t *testing.T) {
	t.Skip("skipped: state snapshot format changed with PoB account fields")
