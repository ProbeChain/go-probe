...
type journalEntry interface {
	...
	// dirtied returns the ProbeChain address modified by this journal entry.
...
func (ch dPosCandidateChange) revert(s *StateDB) {
	validatorListAccount := s.getStateObject(*ch.account).validatorListAccount
	validatorListAccount.ValidatorCandidates = ch.validatorCandidates
	validatorListAccount.RoundId = ch.roundId
}
