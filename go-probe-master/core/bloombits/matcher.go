...
type Matcher struct {
	...
	running uint32 // Atomic flag whprobeer a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togprobeer.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togprobeer
				var orVector []byte
...
// the requested retrieval queue to be serviced togprobeer with other sessions.
//
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, somprobeing must arrive before we're aborting
