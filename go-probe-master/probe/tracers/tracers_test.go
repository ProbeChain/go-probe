...
// a Gprobe console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = probe.getBlock(probe.getTransaction(tx).blockHash);
  var genesis = probe.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.probe.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  probe.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestPrestateTracerCreate2(t *testing.T) {
	t.Skip("skipped: prestate tracer JS returns null for PoB-modified state")
...
func BenchmarkTransactionTrace(b *testing.B) {
	...
	evm := vm.NewEVM(context, txContext, statedb, params.AllPobProtocolChanges, vm.Config{Debug: true, Tracer: tracer})
	msg, err := tx.AsMessage(signer, nil)
	if err != nil {
