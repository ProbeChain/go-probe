...
func (h *handler) syncTransactions(p *probe.Peer) {
	...
	// The probe/65 protocol introduces proper transaction announcements, so instead
...
func (h *handler) txsyncLoop64() {
	...
	var (
		...
		sending = false               // whprobeer a send is active
	...
	send := func(s *txsync) {
		if s.p.Version() >= probe.ETH65 {
			panic("initial transaction syncer running on probe/65+")
		}
...
// chain head.
func (cs *chainSyncer) handlePeerEvent(peer *probe.Peer) bool {
	select {
...
// nextSyncOp determines whprobeer sync is required at this time.
...
func peerToSyncOp(mode downloader.SyncMode, p *probe.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync || op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the fast sync Gprobe won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next fast sync(e.g. user kill Gprobe manually and
		// restart) then it will be hard for Gprobe to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the fast sync. If changed, Gprobe
