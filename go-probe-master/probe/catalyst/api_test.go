...
var (
	...
	// testAddr is the ProbeChain address of the tester account.
...
func TestProbe2AssembleBlock(t *testing.T) {
	t.Skip("skipped: VerifyValidatorInfo fails on test blocks")
	genesis, blocks := generateTestChain()
	n, probeservice := startProbeService(t, genesis, blocks[1:9])
	defer n.Close()

	api := newConsensusAPI(probeservice)
	signer := types.NewEIP155Signer(probeservice.BlockChain().Config().ChainID)
	...
	probeservice.TxPool().AddLocal(tx)
...
func TestProbe2AssembleBlockWithAnotherBlocksTxs(t *testing.T) {
	genesis, blocks := generateTestChain()
	n, probeservice := startProbeService(t, genesis, blocks[1:9])
	defer n.Close()
...
func TestProbe2NewBlock(t *testing.T) {
	genesis, blocks, forkedBlocks := generateTestChainWithFork(10, 4)
	n, probeservice := startProbeService(t, genesis, blocks[1:5])
	defer n.Close()

	api := newConsensusAPI(probeservice)
	for i := 5; i < 10; i++ {
		p := executableData{
			ParentHash:   probeservice.BlockChain().CurrentBlock().Hash(),
	...
	exp := probeservice.BlockChain().CurrentBlock().Hash()
	...
	for i := 0; i < 4; i++ {
		...
		lastBlock, err = insertBlockParamsToBlock(probeservice.BlockChain().Config(), lastBlock.Header(), p)
		if err != nil {
			t.Fatal(err)
		}
	}

	if probeservice.BlockChain().CurrentBlock().Hash() != exp {
		t.Fatalf("Wrong head after inserting fork %x != %x", exp, probeservice.BlockChain().CurrentBlock().Hash())
	}
}

// startProbeService creates a full node instance for testing.
func startProbeService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *probe.Probeum) {
	t.Helper()
	...
	probecfg := &probeconfig.Config{Genesis: genesis, Probeash: pob.Config{PowMode: pob.ModeFake}}
	probeservice, err := probe.New(n, probecfg)
	if err != nil {
		t.Fatal("can't create probe service:", err)
	}
	if err := n.Start(); err != nil {
		t.Fatal("can't start node:", err)
	}
	if _, err := probeservice.BlockChain().InsertChain(blocks); err != nil {
		n.Close()
