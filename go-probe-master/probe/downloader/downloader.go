...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [probe/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	snapSync       bool         // Whprobeer to run state sync over the snap protocol
...
func New(checkpoint uint64, stateDb probedb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() probeum.SyncProgress {
...
// Synchronising returns whprobeer the downloader is currently retrieving blocks.
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	// sync mode. Long term we could drop fast sync or merge the two togprobeer,
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	if p.version < probe.ETH65 {
		return fmt.Errorf("%w: advertized %d < required %d", errTooOld, p.version, probe.ETH65)
	}
	mode := d.getMode()

	log.Debug("Synchronising with the network", "peer", p.id, "probe", p.version, "head", hash, "td", td, "mode", mode)
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (commonAncestor uint64, err error) {
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somprobeing valid
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // Whprobeer the next request is pivot verification
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somprobeing completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somprobeing)
				// R: Nothing to give
				if mode != LightSync {
				...
				// peer gave us somprobeing useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if somprobeing failed in between processing chunks
				select {
				case <-d.cancelCh:
					rollbackErr = errCanceled
					return errCanceled
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put togprobeer a new list of blocks to import, which is a superset
