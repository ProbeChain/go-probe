...
type storageResponse struct {
	...
	cont bool // Whprobeer the last storage range has a continuation
}
...
type accountTask struct {
	...
	needCode  []bool // Flags whprobeer the filling accounts need code retrieval
	needState []bool // Flags whprobeer the filling accounts need storage retrieval
	needHeal  []bool // Flags whprobeer the filling accounts's state was chunked and need healing
...
type Syncer struct {
	db probedb.KeyValueStore // Database to store the trie nodes into (and dedup)
...
func (s *Syncer) Sync(root common.Hash, cancel chan struct{}) error {
	...
	// Whprobeer sync completed or not, disregard any future packets
	defer func() {
	...
	// ephemeral so a data race doesn't accidentally deliver somprobeing stale on
	...
	for {
		...
		// Wait for somprobeing to happen
		select {
		case <-s.update:
			// Somprobeing happened (new peer, delivery, timeout), recheck tasks
		case <-peerJoin:
		...
		// Report stats if somprobeing meaningful happened
		s.report(false)
	}
}
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = probedb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for _, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = probedb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) assignTrienodeHealTasks(success chan *trienodeHealResponse, fail chan *trienodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.trieTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		// togprobeer with bytecodes, so we need to queue them combined.
		var (
...
func (s *Syncer) assignBytecodeHealTasks(success chan *bytecodeHealResponse, fail chan *bytecodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.codeTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		// If there are not enough trie tasks queued to fully assign, fill the
		// queue from the state sync scheduler. The trie synced schedules these
		// togprobeer with trie nodes, so we need to queue them combined.
		var (
			have = len(s.healer.trieTasks) + len(s.healer.codeTasks)
			want = maxTrieRequestCount + maxCodeRequestCount
		)
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > probedb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > probedb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash, accounts [][]byte, proof [][]byte) error {
	...
	// Whprobeer or not the response is valid, we can mark the peer as idle and
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > probedb.IdealBatchSize {
