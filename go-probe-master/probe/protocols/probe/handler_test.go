...
// testBackend is a mock implementation of the live Probeum message handler. Its
// purpose is to allow testing the request/reply workflows and wire serialization
// in the `probe` protocol without actually doing any data processing.
...
func newTestBackendWithGenerator(blocks int, generator func(int, *core.BlockGen)) *testBackend {
	...
	chain, _ := core.NewBlockChain(db, nil, params.TestChainConfig, pob.NewFaker(), vm.Config{}, nil, nil, nil)

	bs, _ := core.GenerateChain(params.TestChainConfig, chain.Genesis(), pob.NewFaker(), db, blocks, generator)
