...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `probe` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `probe`.
...
// NodeInfo represents a short summary of the `probe` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // ProbeChain network ID (1=Mainnet, 1205=ProbeChain)
...
// nodeInfo retrieves some `probe` protocol metadata about the running host node.
...
// Handle is invoked whenever an `probe` connection is made that successfully passes
...
var probe65 = map[uint64]msgHandler{
...
var probe66 = map[uint64]msgHandler{
	NewBlockHashesMsg:             handleNewBlockhashes,
	NewBlockMsg:                   handleNewBlock,
	TransactionsMsg:               handleTransactions,
	NewPooledTransactionHashesMsg: handleNewPooledTransactionHashes,
	BehaviorProofMsg:                  handleBehaviorProofMsg,
	AckMsg:                    handleAckMsg,

	// probe66 messages with request-id
