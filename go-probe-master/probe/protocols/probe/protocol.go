...
// ProtocolName is the official short name of the `probe` protocol used during
// devp2p capability negotiation.
const ProtocolName = "probe"

// ProtocolVersions are the supported versions of the `probe` protocol (first
// is primary).
var ProtocolVersions = []uint{ETH66, ETH65}

// protocolLengths are the number of implemented message corresponding to
// different protocol versions.
var protocolLengths = map[uint]uint64{ETH66: 19, ETH65: 17}

// maxMessageSize is the maximum cap on the size of a protocol message.
const maxMessageSize = 10 * 1024 * 1024

const (
	// Protocol messages in probe/64
	StatusMsg          = 0x00
...
// Packet represents a p2p message in the `probe` protocol.
type Packet interface {
	Name() string // Name returns a string corresponding to the message type.
	Kind() byte   // Kind returns the message type.
}

// StatusPacket is the network packet for the status message for probe/64 and later.
...
// GetBlockHeadersPacket represents a block header query over probe/66
...
// BlockHeadersPacket represents a block header response over probe/66.
...
// GetBlockBodiesPacket represents a block body query over probe/66.
...
// BlockBodiesPacket is the network packet for block content distribution over probe/66.
...
// BlockBodiesRLPPacket66 is the BlockBodiesRLPPacket over probe/66
...
// GetNodeDataPacket represents a trie node data query over probe/66.
...
// NodeDataPacket is the network packet for trie node data distribution over probe/66.
...
// GetReceiptsPacket represents a block receipts query over probe/66.
...
// ReceiptsPacket is the network packet for block receipts distribution over probe/66.
...
// ReceiptsPacket66 is the probe-66 version of ReceiptsRLPPacket
...
// PooledTransactionsPacket is the network packet for transaction distribution over probe/66.
...
// PooledTransactionsRLPPacket66 is the probe/66 form of PooledTransactionsRLPPacket
