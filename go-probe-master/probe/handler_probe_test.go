...
// testProbeHandler is a mock event handler to listen for inbound network requests
// on the `probe` protocol and convert them into a more easily testable form.
...
func (h *testProbeHandler) Chain() *core.BlockChain              { panic("no backing chain") }
func (h *testProbeHandler) StateBloom() *trie.SyncBloom          { panic("no backing state bloom") }
func (h *testProbeHandler) TxPool() probe.TxPool                   { panic("no backing tx pool") }
func (h *testProbeHandler) AcceptTxs() bool                      { return true }
func (h *testProbeHandler) RunPeer(*probe.Peer, probe.Handler) error { panic("not used in tests") }
func (h *testProbeHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testProbeHandler) Handle(peer *probe.Peer, packet probe.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected probe packet type in tests: %T", packet))
	}
}

// Tests that peers are correctly accepted (or rejected) based on the advertised
// fork IDs in the protocol handshake.
func TestForkIDSplit65(t *testing.T) { testForkIDSplit(t, probe.ETH65) }
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, probe.ETH66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		probeNoFork, _ = newHandler(&handlerConfig{
			Database:   dbNoFork,
			Chain:      chainNoFork,
		...
		probeProFork, _ = newHandler(&handlerConfig{
			Database:   dbProFork,
			Chain:      chainProFork,
			TxPool:     newTestTxPool(),
			Network:    1,
	...
	probeNoFork.Start(1000)
	probeProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer probeNoFork.Stop()
	defer probeProFork.Stop()
	...
	peerNoFork := probe.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := probe.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- probeNoFork.runProbePeer(peerProFork, func(peer *probe.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- probeProFork.runProbePeer(peerNoFork, func(peer *probe.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = probe.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = probe.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions65(t *testing.T) { testRecvTransactions(t, probe.ETH65) }
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, probe.ETH66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := probe.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := probe.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runProbePeer(sink, func(peer *probe.Peer) error {
		return probe.Handle((*probeHandler)(handler.handler), peer)
	})
...
func TestSendTransactions65(t *testing.T) { testSendTransactions(t, probe.ETH65) }
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, probe.ETH66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runProbePeer(src, func(peer *probe.Peer) error {
		return probe.Handle((*probeHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 65, 66:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post probe/65")
			}
...
func TestTransactionPropagation65(t *testing.T) { testTransactionPropagation(t, probe.ETH65) }
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, probe.ETH66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := probe.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := probe.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runProbePeer(sourcePeer, func(peer *probe.Peer) error {
			return probe.Handle((*probeHandler)(source.handler), peer)
		})
		go sink.handler.runProbePeer(sinkPeer, func(peer *probe.Peer) error {
			return probe.Handle((*probeHandler)(sink.handler), peer)
		})
	}
...
// Tests that post probe protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := probe.NewPeer(probe.ETH65, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := probe.NewPeer(probe.ETH65, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runProbePeer(local, func(peer *probe.Peer) error {
			return probe.Handle((*probeHandler)(handler.handler), peer)
		})
	}()
...
func TestBroadcastBlock8Peers(t *testing.T) {
	t.Skip("skipped: merkle root mismatch from PoB state changes")
	testBroadcastBlock(t, 9, 3)
}
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testProbeHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testProbeHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := probe.NewPeer(probe.ETH65, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := probe.NewPeer(probe.ETH65, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock65(t *testing.T) { testBroadcastMalformedBlock(t, probe.ETH65) }
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, probe.ETH66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := probe.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := probe.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runProbePeer(src, func(peer *probe.Peer) error {
		return probe.Handle((*probeHandler)(source.handler), peer)
	})
