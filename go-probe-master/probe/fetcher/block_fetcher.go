...
var (
	blockAnnounceInMeter   = metrics.NewRegisteredMeter("probe/fetcher/block/announces/in", nil)
	blockAnnounceOutTimer  = metrics.NewRegisteredTimer("probe/fetcher/block/announces/out", nil)
	blockAnnounceDropMeter = metrics.NewRegisteredMeter("probe/fetcher/block/announces/drop", nil)
	blockAnnounceDOSMeter  = metrics.NewRegisteredMeter("probe/fetcher/block/announces/dos", nil)

	blockBroadcastInMeter   = metrics.NewRegisteredMeter("probe/fetcher/block/broadcasts/in", nil)
	blockBroadcastOutTimer  = metrics.NewRegisteredTimer("probe/fetcher/block/broadcasts/out", nil)
	blockBroadcastDropMeter = metrics.NewRegisteredMeter("probe/fetcher/block/broadcasts/drop", nil)
	blockBroadcastDOSMeter  = metrics.NewRegisteredMeter("probe/fetcher/block/broadcasts/dos", nil)

	headerFetchMeter = metrics.NewRegisteredMeter("probe/fetcher/block/headers", nil)
	bodyFetchMeter   = metrics.NewRegisteredMeter("probe/fetcher/block/bodies", nil)

	headerFilterInMeter  = metrics.NewRegisteredMeter("probe/fetcher/block/filter/headers/in", nil)
	headerFilterOutMeter = metrics.NewRegisteredMeter("probe/fetcher/block/filter/headers/out", nil)
	bodyFilterInMeter    = metrics.NewRegisteredMeter("probe/fetcher/block/filter/bodies/in", nil)
	bodyFilterOutMeter   = metrics.NewRegisteredMeter("probe/fetcher/block/filter/bodies/out", nil)
)
...
type BlockFetcher struct {
	light bool // The indicator whprobeer it's a light fetcher or normal one.
...
func (f *BlockFetcher) importHeaders(peer string, header *types.Header) {
	...
	go func() {
		...
		// Validate the header and if somprobeing went wrong, drop the peer
...
func (f *BlockFetcher) importBlocks(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somprobeing went very wrong, drop the peer
