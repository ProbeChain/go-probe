...
func TestTransactionFetcherWaiting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			// Initial announcement to get somprobeing into the waitlist
...
func TestTransactionFetcherOutOfBoundDeliveries(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			// Deliver somprobeing out of the blue
			isWaiting(nil),
			isScheduled{nil, nil, nil},
...
func testTransactionFetcher(t *testing.T, tt txFetcherTest) {
	...
	for i, step := range tt.steps {
		...
		case doWait:
			clock.Run(step.time)
			if step.step {
				<-wait // Fetcher supposed to do somprobeing, wait until it's done
			}
...
// containsHash returns whprobeer a hash is contained within a hash slice.
