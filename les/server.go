...
type LesServer struct {
	...
	archiveMode bool // Flag whprobeer the probeum node runs in archive mode.
...
func NewLesServer(node *node.Node, e probeBackend, config *probeconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "probe/db/lesserver/", false)
	if err != nil {
		return nil, err
	}
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "Probeum light client service")
