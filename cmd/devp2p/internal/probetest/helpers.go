...
// Is_66 checks if the node supports the probe66 protocol version,
...
func (s *Suite) dial() (*Conn, error) {
	...
	conn.caps = []p2p.Cap{
		{Name: "probe", Version: 64},
		{Name: "probe", Version: 65},
	}
...
// returning the created Conn with additional probe66 capabilities if
// successful
func (s *Suite) dial66() (*Conn, error) {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "probe", Version: 66})
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		c.negotiateProbeProtocol(msg.Caps)
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("unexpected probe protocol version")
		}
		return nil
	default:
...
// negotiateProbeProtocol sets the Conn's probe protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateProbeProtocol(caps []p2p.Cap) {
	var highestProbeVersion uint
	for _, capability := range caps {
		if capability.Name != "probe" {
			continue
		}
		if capability.Version > highestProbeVersion && capability.Version <= c.ourHighestProtoVersion {
			highestProbeVersion = capability.Version
		}
	}
	c.negotiatedProtoVersion = highestProbeVersion
}
...
loop:
	...
	// make sure probe protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("probe protocol version must be set in Conn")
	}
	if status == nil {
...
func (s *Suite) createSendAndRecvConns(isProbe66 bool) (*Conn, *Conn, error) {
	var (
		sendConn *Conn
		recvConn *Conn
		err      error
...
// readAndServe66 serves probe66 GetBlockHeaders requests while waiting
// on another message from the node.
func (c *Conn) readAndServe66(chain *Chain, timeout time.Duration) (uint64, Message) {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			...
			resp := &probe.BlockHeadersPacket66{
				RequestId:          reqID,
				BlockHeadersPacket: probe.BlockHeadersPacket(headers),
			}
...
func (c *Conn) headersRequest(request *GetBlockHeaders, chain *Chain, isProbe66 bool, reqID uint64) (BlockHeaders, error) {
	defer c.SetReadDeadline(time.Time{})
	c.SetReadDeadline(time.Now().Add(20 * time.Second))
	// if on probe66 connection, perform probe66 GetBlockHeaders request
	if isProbe66 {
...
// getBlockHeaders66 executes the given `GetBlockHeaders` request over the probe66 protocol.
func getBlockHeaders66(chain *Chain, conn *Conn, request *GetBlockHeaders, id uint64) (BlockHeaders, error) {
	// write request
	packet := probe.GetBlockHeadersPacket(*request)
	req := &probe.GetBlockHeadersPacket66{
		RequestId:             id,
...
// headersMatch returns whprobeer the received headers match the given request
...
func (s *Suite) sendNextBlock(isProbe66 bool) error {
	// set up sending and receiving connections
	sendConn, recvConn, err := s.createSendAndRecvConns(isProbe66)
	if err != nil {
		return err
	}
	...
	if err = s.waitForBlockImport(recvConn, nextBlock, isProbe66); err != nil {
...
func (s *Suite) waitForBlockImport(conn *Conn, block *types.Block, isProbe66 bool) error {
	...
	for {
		...
		if isProbe66 {
			requestID := uint64(54)
			headers, err = conn.headersRequest(req, s.chain, probe66, requestID)
		} else {
...
func (s *Suite) oldAnnounce(isProbe66 bool) error {
	sendConn, receiveConn, err := s.createSendAndRecvConns(isProbe66)
	if err != nil {
		return err
	}
	defer sendConn.Close()
...
func (s *Suite) maliciousHandshakes(t *utesting.T, isProbe66 bool) error {
	var (
		conn *Conn
		err  error
	)
...
func (s *Suite) hashAnnounce(isProbe66 bool) error {
	// create connections
	sendConn, recvConn, err := s.createSendAndRecvConns(isProbe66)
	if err != nil {
	...
	if isProbe66 {
		...
		case GetBlockHeaders:
			...
			resp := &probe.BlockHeadersPacket66{
				RequestId: id,
				BlockHeadersPacket: probe.BlockHeadersPacket{
					nextBlock.Header(),
				},
			}
