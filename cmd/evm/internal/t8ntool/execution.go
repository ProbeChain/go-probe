...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig,
	...
	// If DAO is supported/enabled, we need to handle it here. In gprobe 'proper', it's
	...
	for i, tx := range txs {
		...
		// Receipt:
		{
			...
			if chainConfig.IsByzantium(vmContext.BlockNumber) {
				...
				root = statedb.IntermediateRoot(chainConfig.IsEIP158(vmContext.BlockNumber), vmContext.BlockNumber).Bytes()
			}
	...
	statedb.IntermediateRoot(chainConfig.IsEIP158(vmContext.BlockNumber), vmContext.BlockNumber)
	// Add mining reward?
	if miningReward > 0 {
...
func MakePreState(db probedb.Database, accounts core.GenesisAlloc) *state.StateDB {
