...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string, readonly bool) (probedb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string, readonly bool) (probedb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) wrapDatabase(db probedb.Database) probedb.Database {
