...
func TestGenerate(t *testing.T) {
	//Create an account
	key, err := probecrypto.GenerateKeyByType(0x00)
	if err != nil {
		fmt.Println("Error: ", err.Error())
	}
	hexPriKey := hex.EncodeToString(probecrypto.FromECDSA(key))
	//不含0x的私钥65
	fmt.Printf("private key [%d] [%v]\n", len(hexPriKey), hexPriKey)
	//Get the address
	address := probecrypto.PubkeyToAddress(key.PublicKey).Hex()
	fmt.Printf("address[%d][%v]\n", len(address), address)
	//	address2 := PubkeyToNormalAddress(key.PublicKey).Hex()
	//fmt.Printf("address[%d][%v]\n", len(address2), address2)

	acc1Key, _ := probecrypto.HexToECDSA(hexPriKey)
	//probecrypto.FromECDSAPub(&key)[1:]
	fmt.Println("public key from prikey  \n", hexutil.Encode(probecrypto.FromECDSAPub(&acc1Key.PublicKey)))
	address1 := probecrypto.PubkeyToAddress(acc1Key.PublicKey).Hex()
...
func TestCreateAddressForAccountType(t *testing.T) {
	//Create an account
	k := byte(0x02)
	key, err := probecrypto.GenerateKeyByType(k)
	if err != nil {
		fmt.Println("Error: ", err.Error())
	}
	hexPriKey := hex.EncodeToString(probecrypto.FromECDSA(key))
	...
	fmt.Printf("flag[%T][%X]\n", c, c)
	address_02, _ := probecrypto.CreateAddressForAccountType(address, uint64(123456), 0x02)
...
func TestSign01(*testing.T) {
	...
	hexPriKey := hex.EncodeToString(probecrypto.FromECDSA(key))
	//带有0x的私钥
	fmt.Println("private key have 0x   n", hexutil.Encode(probecrypto.FromECDSA(key)))
	//不含0x的私钥65
	fmt.Printf("private key [%d] [%v]\n", len(hexPriKey), hexPriKey)
	//Get the address
	address := probecrypto.PubkeyToAddress(key.PublicKey)
	...
	key1, _ := probecrypto.HexToECDSA(hexPriKey)
	sig, _ := crypto.Sign(digestHash, key1)
	fmt.Println("sig ", hex.EncodeToString(sig))
	recoveredPub, _ := crypto.Ecrecover(digestHash, sig)
	pubKey, _ := probecrypto.UnmarshalPubkey(recoveredPub)
	recoveredAddr := probecrypto.PubkeyToAddress(*pubKey)

	recoveredPub2, _ := crypto.SigToPub(digestHash, sig)
	recoveredAddr2 := probecrypto.PubkeyToAddress(*recoveredPub2)
	tt := probecrypto.FromECDSAPub(&key1.PublicKey)
	//addrtest:=hexutil.Encode(probecrypto.FromECDSAPub(&key1.PublicKey))
...
func TestSign(*testing.T) {
	...
	key1, _ := probecrypto.HexToECDSA(hexPriKey)
	//addrtest := probecrypto.PubkeyToAddressForType(key1.PublicKey, k).Hex()
	//测试数据签名
	msg := probecrypto.Keccak256([]byte("foo"))
	sig, err := probecrypto.Sign(msg, key1)
	if err != nil {
		fmt.Println("Error: ", err.Error())
	}
	k = sig[len(sig)-1]
	sig = sig[:len(sig)-1]
	recoveredPub, err := probecrypto.Ecrecover(msg, sig)
	pubKey, _ := probecrypto.UnmarshalPubkeyForType(recoveredPub, k)
	recoveredAddr := probecrypto.PubkeyToAddressForType(*pubKey, k)

	// should be equal to SigToPub
	recoveredPub2, _ := probecrypto.SigToPubForType(msg, sig, k)
	recoveredAddr2 := probecrypto.PubkeyToAddressForType(*recoveredPub2, k)
	//验证签名
	//fmt.Println("addrtest ", addrtest)
	fmt.Println("recoveredAddr ", recoveredAddr.String())
	fmt.Println("recoveredAddr2 ", recoveredAddr2.String())*/

	key, err := probecrypto.GenerateKeyByType(0x03)
	if err != nil {
		fmt.Println("failed GenerateKey with: ", err.Error())
	}

	fmt.Println("private key have 0x   \n", hexutil.Encode(probecrypto.FromECDSA(key)))
	fmt.Println("private key no 0x \n", hex.EncodeToString(probecrypto.FromECDSA(key)))

	/*if err := probecrypto.SaveECDSA("privatekey", key); err != nil {
		log.Error(fmt.Sprintf("Failed to persist node key: %v", err))
	}*/

	fmt.Println("public key have 0x   \n", hexutil.Encode(probecrypto.FromECDSAPub(&key.PublicKey)))
	fmt.Println("public key no 0x \n", hex.EncodeToString(probecrypto.FromECDSAPub(&key.PublicKey)))

	//由私钥字符串转换私钥
	acc1Key, _ := probecrypto.HexToECDSA("0342875f5170623c91e184b9e91d7c1dd381d4e3a9af9ccba2e656626005baf21a")
	address1 := probecrypto.PubkeyToAddress(acc1Key.PublicKey)
	...
	key1, _ := probecrypto.HexToECDSA(testPrivHex)
...
func TestHexToAddress(*testing.T) {
	key, err := probecrypto.GenerateKeyByType(0x04)
	if err != nil {
		fmt.Println("failed GenerateKey with: ", err.Error())
	}
	...
	fmt.Println("public key have 0x   \n", hexutil.Encode(probecrypto.FromECDSAPub(&key.PublicKey)))
	fmt.Println("public key no 0x \n", hex.EncodeToString(probecrypto.FromECDSAPub(&key.PublicKey)))
	oldPkAddr := probecrypto.PubkeyToAddress(key.PublicKey)
...
func TestStrToHex(*testing.T) {
	...
	var emptyCodeHash2 = probecrypto.Keccak256(nil)
	fmt.Println("address ", hexutil.Encode(emptyCodeHash2))*/
}

func TestPrivateKey(*testing.T) {
	acc1Key, _ := probecrypto.HexToECDSA("00a90258933be28f5890ebd7a57e2641e63c15078a2b1cf043cba8be43c0d36f58")
	address1 := probecrypto.PubkeyToAddress(acc1Key.PublicKey)
	fmt.Println("address ", address1.String())
}

func TestEnode(*testing.T) {
	acc1Key, _ := probecrypto.HexToECDSA("000e92781f2a5f582a7ce9726971ce896421080b6cb9fe33cfe892968e7d25fd85")
	address1 := probecrypto.PubkeyToAddress(acc1Key.PublicKey)
	fmt.Println("address ", address1.String())
	nodeId := fmt.Sprintf("%x", probecrypto.FromECDSAPub(&acc1Key.PublicKey)[1:])
...
func TestPrintDposNode(*testing.T) {
	//privateKey, _ := probecrypto.GenerateKey()
	privateKey, _ := probecrypto.HexToECDSA("00bfed0787f3a2c033177bf18a89717a64b3a38eda85b20ce2c65b02b5ea75079a")
	fmt.Println("private key have 0x   \n", hexutil.Encode(probecrypto.FromECDSA(privateKey)))
	address := probecrypto.PubkeyToAddress(privateKey.PublicKey)
	...
	fmt.Println("public key have 0x   \n", hexutil.Encode(probecrypto.FromECDSAPub(&privateKey.PublicKey)))
...
func TestJS2GOSign(*testing.T) {
	...
	//由私钥字符串转换私钥
	acc1Key, _ := probecrypto.HexToECDSA("00a958379c166d74f7280e72bf4c62e53fc49efc7f74b08b8d18a5c955fa889a62")
	address1 := probecrypto.PubkeyToAddress(acc1Key.PublicKey)
	fmt.Println("address ", address1.String())
	fmt.Println("************************** ")
	msg3 := probecrypto.Keccak256([]byte("foo"))
	...
	msg2 := probecrypto.Keccak512([]byte("Some data"))
	fmt.Println("hexSig ", string(msg2[:]))
	msg := probecrypto.Keccak256([]byte("root"))
...
func TestAccounTypeFoGenrateSign(*testing.T) {
	...
	nonce := uint64(2)
	contractAddr, _ := probecrypto.CreateAddressForAccountType(toAddress, nonce, common.ACC_TYPE_OF_CONTRACT)
...
func TestOfflineSign(*testing.T) {
	var testAddrHex = "0028c10835389d35319bcd8e5e21857b4832bbd77ba47af098"
	var testPrivHex = "00a958379c166d74f7280e72bf4c62e53fc49efc7f74b08b8d18a5c955fa889a62"
	privateKey, _ := probecrypto.HexToECDSA(testPrivHex)
	toAddress := common.HexToAddress(testAddrHex)
