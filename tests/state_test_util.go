...
// See https://github.com/probeum/EIPs/issues/176 for the test format specification.
...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool) (*snapshot.Tree, *state.StateDB, common.Hash, error) {
	...
	if config.IsLondon(new(big.Int)) {
		...
		if baseFee == nil {
			// Retestprobe uses `0x10` for genesis baseFee. Therefore, it defaults to
...
func MakePreState(db probedb.Database, accounts core.GenesisAlloc, snapshotter bool) (*snapshot.Tree, *state.StateDB) {
...
func (tx *stTransaction) toMessage(ps stPostState, baseFee *big.Int) (core.Message, error) {
	...
	// Value, Data hex encoding is messy: https://github.com/probeum/tests/issues/203
	value := new(big.Int)
	if valueHex != "0x" {
	...
	msg := types.NewMessage(from, to, 0, tx.Nonce, value, gasLimit, gasPrice,
		tx.MaxFeePerGas, tx.MaxPriorityFeePerGas, data, accessList, true, nil)
	return msg, nil
}
