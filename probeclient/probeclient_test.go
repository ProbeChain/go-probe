...
// Verify that Client implements the probeum interfaces.
var (
	_ = probeum.ChainReader(&Client{})
	_ = probeum.TransactionReader(&Client{})
	_ = probeum.ChainStateReader(&Client{})
	_ = probeum.ChainSyncReader(&Client{})
	_ = probeum.ContractCaller(&Client{})
	_ = probeum.GasEstimator(&Client{})
	_ = probeum.GasPricer(&Client{})
	_ = probeum.LogFilterer(&Client{})
	_ = probeum.PendingStateReader(&Client{})
	// _ = probeum.PendingStateEventer(&Client{})
	_ = probeum.PendingContractCaller(&Client{})
)
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == probeum.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != probeum.NotFound {
		t.Fatal("error should be probeum.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AllProbeashProtocolChanges.ChainID) != 0 {
