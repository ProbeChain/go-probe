...
func TestDPosCandidate(t *testing.T) {
	...
	for i, aa := range dPosCandidateAccounts {
		fmt.Printf("%d, Owner:%s,Vote:%s,Weight:%d,VoteValue:%d,Enode:%s\n", i+1, aa.Owner, aa.Vote, aa.Weight, aa.VoteValue, parseIp(aa.Enode))
	}
...
func TestDPosCandidate2(t *testing.T) {
	a1 := common.DPoSCandidateAccount{common.BytesToDposEnode([]byte("enode://@192.168.0.4:80")), [25]byte{1}, [25]byte{1}, big.NewInt(1), big.NewInt(100)}
	a2 := common.DPoSCandidateAccount{common.BytesToDposEnode([]byte("enode://@192.168.0.4:80")), [25]byte{1}, [25]byte{2}, big.NewInt(1), big.NewInt(200)}
	a3 := common.DPoSCandidateAccount{common.BytesToDposEnode([]byte("enode://@192.168.0.4:80")), [25]byte{1}, [25]byte{3}, big.NewInt(1), big.NewInt(300)}
