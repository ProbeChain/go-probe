...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	chain, err := NewBlockChain(db, cacheConfig, params.AllProbeashProtocolChanges, engine, vm.Config{}, nil, nil)
	if err != nil {
...
// It can be used for testing that restart Gprobe normally.
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(snaptest.db, nil, params.AllProbeashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
...
// It can be used for testing that restart Gprobe after the crash.
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(newdb, nil, params.AllProbeashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, cacheConfig, params.AllProbeashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, config, params.AllProbeashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newBlocks, _ := GenerateChain(params.TestChainConfig, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.newBlocks, func(i int, b *BlockGen) {})
...
// Tests a Gprobe restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Gprobe was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Gprobe was crashed and restarts with a broken snapshot. In this case
...
// Tests a Gprobe was running with snapshot enabled. Then restarts without
...
// Tests the Gprobe was running with snapshot enabled and resetHead is applied.
...
// Tests the Gprobe was running with a complete snapshot and then imports a few
