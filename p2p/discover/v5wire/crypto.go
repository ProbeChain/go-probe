...
func EncodePubkey(key *probe.PublicKey) []byte {
	switch key.Curve {
	case crypto.S256():
...
func DecodePubkey(curve elliptic.Curve, e []byte) (*probe.PublicKey, error) {
	switch curve {
	case crypto.S256():
		if len(e) != 33 {
...
func makeIDSignature(hash hash.Hash, key *probe.PrivateKey, challenge, ephkey []byte, destID enode.ID) ([]byte, error) {
...
func deriveKeys(hash hashFn, priv *probe.PrivateKey, pub *probe.PublicKey, n1, n2 enode.ID, challenge []byte) *session {
...
func ecdh(privkey *probe.PrivateKey, pubkey *probe.PublicKey) []byte {
