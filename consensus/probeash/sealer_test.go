...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom probeash engine.
	probeash := NewTester([]string{server.URL}, false)
	...
	probeash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := probeash.SealHash(header).Hex(); work[0] != want {
...
// Tests whprobeer remote HTTP servers are correctly notified of new work. (Full pending block body / --miner.notify.full)
func TestRemoteNotifyFull(t *testing.T) {
	...
	probeash := New(config, []string{server.URL}, false)
	defer probeash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom probeash engine.
	probeash := NewTester([]string{server.URL}, false)
	probeash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer probeash.Close()
...
// Tests whprobeer stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	probeash := NewTester(nil, true)
	defer probeash.Close()
	api := &API{probeash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			probeash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, probeash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch probeash result timeout", id+1)
		}
	}
}
